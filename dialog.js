...
const Dialog = (() => {
  ...
  function show(speakerName, portraitEmoji, dialogLines, callback = null) {
    ...
    idx      = 0;
    onDone   = callback;
    portrait = portraitEmoji;
    speaker  = speakerName;

    document.getElementById('dialogPortrait').textContent = portrait;
    document.getElementById('dialogName').textContent     = speaker;
  ...
  function next() {
    ...
    if (idx >= lines.length) {
      ...
      document.getElementById('dialogText').textContent = lines[idx];
    }
  }
